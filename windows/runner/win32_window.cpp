...
// forward declarations (чтобы вызовы были видны до определений ниже)
static void StopVpnFast();
static void DisableSystemProxyFast();
                            ...
                            LPARAM const lparam) noexcept
{
  ...
  case WM_QUERYENDSESSION:
    // Система завершает сеанс (выключение/перезагрузка/лог-офф):
    StopVpnFast();
    DisableSystemProxyFast();
...
static void StopVpnFast()
{
  // Путь к CLI рядом с .exe приложения
  wchar_t modulePath[MAX_PATH];
  GetModuleFileNameW(NULL, modulePath, MAX_PATH);
  std::wstring exeDir(modulePath);
  exeDir = exeDir.substr(0, exeDir.find_last_of(L"\\/"));
  std::wstring cli = exeDir + L"\\RostovVPNCli.exe";

  // 1) мягко
  ShellExecuteW(NULL, L"open", cli.c_str(), L"tunnel stop", exeDir.c_str(), SW_HIDE);
...
static void DisableSystemProxyFast()
{
  ...
  std::wstring netsh = std::wstring(sysdir) + L"\\netsh.exe";
  ShellExecuteW(NULL, L"open", netsh.c_str(), L"winhttp reset proxy",
                sysdir, SW_HIDE);
}